...
void MultiLayerNeuralNetwork::initialize(float alpha, errorMeasure e, int depth, int* dims)  {
    std::vector<CuMatrix> weights(depth);
    std::vector<CuMatrix> biases(depth);
...
void MultiLayerNeuralNetwork::predict(CuMatrix<float> &input, CuMatrix<char> &output) {
    ...
    for (int l = 1; l < depth; l++) {
        CuMatrix<float> yn(dims[l], n)
        forwardPropagate(y, yn);
        y = yn;
    }
    CuMatrix<float> yn(dims[depth], n)
...
void MultiLayerNeuralNetwork::runTrainingIteration(CuMatrix<float> &data, CuMatrix<float> &labels) {
    ...
    y_vals.at(0) = data;
    for (int l = 1; l < depth; l++) {
        CuMatrix<float> y(dims[l], n);
        forwardPropagate(y_vals.at(l-1), y);
        y_vals.at(l) = y;
    }
    CuMatrix<float> y(dims[depth], n);
    sigmoidForwardPropagate(y_vals.at(depth-1), y);

    std::vector<CuMatrix> dls(depth);
    // Calculate delta for last layer
    CuMatrix<float> dl(dims[depth], n);
    switch (fError) {
    case MEAN_SQUARED:
    {
        // Calculate (y-t)
        CuMatrix<float> ymt(dims[depth], n);
    ...
    for (int l = depth; l > 1; l--) {
        CuMatrix<float> dln(dims[l-1], n);
        // Calculate (W(l)*d(l))
        CuMatrix<float>::multiply(weights.at(l-1), false, dls.at(l-1), false, dln)
        // Calculate (1-y(l-1)^2)
        CuMatrix<float> yn(dims[l-1], n);
        CuMatrix<float> one(dims[l-1], n);
    ...
    for (int l = 0; l < depth; l++) {
        // Update W(l)
        CuMatrix<float> dW(dims[l], dims[l+1]);
